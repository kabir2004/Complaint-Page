...
export async function sendVerificationEmail(email: string, token: string, complaintId: string) {
  ...
  try {
    ...
    await logEmail(complaintId, 'VERIFICATION', email, fromEmail, subject, htmlBody, 'FAILED', error instanceof Error ? error.message : 'Unknown error')
    throw error
  }
}
...
export async function sendInternalEmail(complaint: any) {
  ...
  try {
    ...
    await logEmail(complaint.id, 'INTERNAL', toEmail, fromEmail, subject, htmlBody, 'FAILED', error instanceof Error ? error.message : 'Unknown error')
    throw error
  }
}
...
export async function sendAcknowledgementEmail(complaint: any) {
  ...
  try {
    ...
    await logEmail(complaint.id, 'ACKNOWLEDGEMENT', complaint.email, fromEmail, subject, htmlBody, 'FAILED', error instanceof Error ? error.message : 'Unknown error')
    throw error
  }
}
