...
// Generic function to send emails via Supabase Edge Function
async function sendEmailViaSupabase(
  ...
  const { data, error } = await supabase.functions.invoke('send-email', {
    body: {
      ...
      subject,
      html,
      from: from || 'Sterling Mutuals <complaintssterling@gmail.com>',
    },
  })
...
export async function sendVerificationEmail(email: string, token: string, complaintId: string) {
  ...
  const fromEmail = 'Sterling Mutuals <complaintssterling@gmail.com>'
  ...
  try {
    const result = await sendEmailViaSupabase(email, subject, htmlBody, fromEmail)
...
export async function sendInternalEmail(complaint: any) {
  const fromEmail = 'Sterling Mutuals <complaintssterling@gmail.com>'
  const toEmail = 'complaintssterling@gmail.com'
  ...
  try {
    const result = await sendEmailViaSupabase(toEmail, subject, htmlBody, fromEmail)
...
export async function sendAcknowledgementEmail(complaint: any) {
  ...
  try {
    const result = await sendEmailViaSupabase(complaint.email, subject, htmlBody, fromEmail)
